...
from .comfy_trace.model_base import BaseModelApplyModelModuleFactory, BaseModelApplyModelModuleFactoryCus
...
class LazyTraceModuleCus:
    ...
    def __call__(self, model_function, /, **kwargs):
        module_factory = BaseModelApplyModelModuleFactoryCus(model_function, kwargs)
        kwargs = module_factory.get_converted_kwargs()
        key = module_factory.gen_cache_key()
...
def build_lazy_trace_module_cus(config, device, patch_id):
    config.enable_cuda_graph = config.enable_cuda_graph and device.type == "cuda"

    if config.enable_xformers:
        _enable_xformers(None)

    return LazyTraceModuleCus(
        config=config,
        patch_id=patch_id,
        check_trace=True,
        strict=True,
