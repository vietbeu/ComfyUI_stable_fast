...
class BaseModelApplyModelModuleCus(torch.nn.Module):
    def __init__(self, func, module):
        super().__init__()
        self.func = func
        self.module = module
...
class BaseModelApplyModelModuleFactoryCus(ModuleFactory):
    kwargs_name = (
        "input_x",
        "timestep",
        "c_concat",
    ...
    @contextlib.contextmanager
    def converted_module_context(self):
        module = BaseModelApplyModelModuleCus(self.callable, self.callable.__self__)
        if len(self.patch_module) > 0:
            self.callable.__self__.diffusion_model = PatchUNetModel.cast_from(
                self.callable.__self__.diffusion_model
            )
